...
func (self *Gate) Enter() bool {
	if self == nil {
		return false
	}
	for {
		v := atomic.LoadUint32(&self.userCount)
		if v&gateClosed != 0 {
			return false
		}
		if atomic.CompareAndSwapUint32(&self.userCount, v, v+1) {
			return true
		}
	}
}
...
func (self *Gate) Close() {
	for {
		v := atomic.LoadUint32(&self.userCount)
		if v&^gateClosed != 0 && self.done == nil {
			self.done = make(chan struct{})
		}
		if atomic.CompareAndSwapUint32(&self.userCount, v, v|gateClosed) {
			if v&^gateClosed != 0 {
				<-self.done
			}
			return
