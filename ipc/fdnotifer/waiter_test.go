...
func (self *callbackStub) Callback(entry *Entry) { self.f(entry) }
...
func TestMask(t *testing.T) {
	...
	if cnt != 1 {
		t.Errorf("[error] callback wasn't called when it should have been")
	}
	...
	q.Notify(EventIn)
	if cnt != 1 {
		t.Errorf("[error] allback wasn't called when it should have been")
	}
	...
	if cnt != 0 {
		t.Errorf("[error] callback was called when it shouldn't have been")
	}
}
...
func TestConcurrentRegistration(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		go func() {
			...
			e.Callback = &callbackStub{func(entry *Entry) {
				cnt++
				if entry != &e {
					t.Errorf("[error] entry = %p, want %p", entry, &e)
				}
			}}
	...
	q.Notify(EventIn)
	if cnt != concurrency {
		t.Errorf("[error] cnt = %d, want %d", cnt, concurrency)
	}
...
func TestConcurrentNotification(t *testing.T) {
	...
	if cnt != concurrency*waiterCount {
		t.Errorf("[error] cnt = %d, want %d", cnt, concurrency*waiterCount)
	}
}
