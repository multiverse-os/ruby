...
func (ep *Endpoint) RecvFD() (int, error)         { return ep.recvFD(0) }
func (ep *Endpoint) RecvFDNonblock() (int, error) { return ep.recvFD(syscall.MSG_DONTWAIT) }
...
func (ep *Endpoint) recvFD(flags uintptr) (int, error) {
	...
	if int(ep.msghdr.Controllen) != cmsgLen {
		return -1, fmt.Errorf("[error] received control message has incorrect length: got %d, wanted %d", ep.msghdr.Controllen, cmsgLen)
	}
	if ep.cmsg.Level != syscall.SOL_SOCKET || ep.cmsg.Type != syscall.SCM_RIGHTS {
		return -1, fmt.Errorf("[error] received control message has incorrect (level, type): got (%v, %v), wanted (%v, %v)", ep.cmsg.Level, ep.cmsg.Type, syscall.SOL_SOCKET, syscall.SCM_RIGHTS)
	}
